...
function playAShuffledSong() {
  const musicWithoutCurrentSong = musicData.filter(
    (el) => el.id !== currentMusicIndex,
  );
  const randomMusic =
    musicWithoutCurrentSong[
      Math.trunc(Math.random() * musicWithoutCurrentSong.length)
    ];
